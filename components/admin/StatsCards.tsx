import { Users, Clock3, CheckCircle, BellRing } from 'lucide-react';
...
export default function StatsCards({ stats }: any) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 shrink-0">
      ...
      <div className="bg-[#605ca8] rounded-sm p-4 flex items-center justify-between text-white shadow-sm">
        <div>
          <p className="text-3xl font-bold mb-0">{stats?.registrations || 0}</p>
          <p className="text-[10px] font-bold uppercase tracking-widest text-white/80">Data Masuk</p>
        </div>
        <BellRing size={40} className="text-white/20" />
      </div>
    </div>
  );
}