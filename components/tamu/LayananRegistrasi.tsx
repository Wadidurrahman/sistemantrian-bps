    ...
    <main className="min-h-dvh bg-gradient-to-b from-[#ea580c] to-[#c2410c] flex flex-col items-center relative overflow-x-hidden font-sans sm:justify-center">
      ...
      <div className={`w-full max-w-sm px-6 pt-12 pb-24 relative z-10 text-left transition-all duration-1000 transform ${mounted ? 'translate-y-0 opacity-100' : '-translate-y-8 opacity-0'}`}>
        ...
        <p className="text-orange-50 text-sm font-medium leading-relaxed drop-shadow-md">
          Pendaftaran layanan dan akun integrasi BPS.
        </p>
      </div>
        ...
        <div className="overflow-y-auto max-h-[65vh] pb-4 px-1 custom-scrollbar">
          ...
          {view === 'pilih' && (
            <div className="space-y-2 pt-2">
              ...
              <div className="flex flex-col gap-3">
                <button onClick={() => setView('individu')} className="w-full flex items-center gap-3 p-4 rounded-xl border-2 border-slate-100 bg-white text-slate-600 hover:border-[#ea580c] hover:bg-orange-50/50 shadow-sm text-left transition-all">
                ...
                <button onClick={() => setView('group')} className="w-full flex items-center gap-3 p-4 rounded-xl border-2 border-slate-100 bg-white text-slate-600 hover:border-[#ea580c] hover:bg-orange-50/50 shadow-sm text-left transition-all">
                ...
                <button onClick={() => setView('lainnya')} className="w-full flex items-center gap-3 p-4 rounded-xl border-2 border-slate-100 bg-white text-slate-600 hover:border-[#ea580c] hover:bg-orange-50/50 shadow-sm text-left transition-all">
                ...
                <button onClick={onBack} className="w-full mt-3 bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold py-3.5 rounded-xl transition-all duration-200 active:scale-95 text-sm tracking-widest uppercase">
                  KEMBALI KE MENU
                </button>
              </div>
            </div>
          ...
          {view === 'individu' && (
            <form onSubmit={(e) => handleSubmit(e, 'Individu')} className="grid grid-cols-1 gap-3 pt-2">
              <div><label className="text-[11px] font-bold text-slate-500 uppercase">Nama Lengkap</label><input required name="Nama Lengkap" onChange={(e) => setIndData({...indData, [e.target.name]: e.target.value})} className="w-full bg-slate-50 border-2 border-slate-100 rounded-xl px-4 py-3 outline-none focus:border-[#ea580c] text-sm font-bold text-slate-800" /></div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><label className="text-[11px] font-bold text-slate-500 uppercase">Gender</label><select name="Jenis Kelamin" onChange={(e) => setIndData({...indData, [e.target.name]: e.target.value})} className="w-full bg-slate-50 border-2 border-slate-100 rounded-xl px-4 py-3 outline-none focus:border-[#ea580c] text-sm font-bold text-slate-800"><option>Laki-laki</option><option>Perempuan</option></select></div>
                <div><label className="text-[11px] font-bold text-slate-500 uppercase">Disabilitas</label><select name="Disabilitas" onChange={(e) => setIndData({...indData, [e.target.name]: e.target.value})} className="w-full bg-slate-50 border-2 border-slate-100 rounded-xl px-4 py-3 outline-none focus:border-[#ea580c] text-sm font-bold text-slate-800"><option>Tidak</option><option>Ya</option></select></div>
              </div>
              <div className="grid grid-cols-2 gap-3">
              ...
              <div className="flex flex-col gap-3 mt-4">
                <button type="submit" disabled={isLoading} className="w-full bg-[#ea580c] text-white font-extrabold py-3.5 rounded-xl shadow-[0_8px_20px_-6px_rgba(249,115,22,0.6)] flex justify-center uppercase tracking-widest text-sm">{isLoading ? <Loader2 size={20} className="animate-spin" /> : 'KIRIM DATA'}</button>
                <button type="button" onClick={() => setView('pilih')} className="w-full bg-slate-100 text-slate-600 font-bold py-3.5 rounded-xl text-sm tracking-widest uppercase active:scale-95 transition-all">KEMBALI</button>
              </div>
            </form>
          )}
          ...
          {view === 'group' && (
            <form onSubmit={(e) => handleSubmit(e, 'Group')} className="grid grid-cols-1 gap-3 pt-2">
              <div><label className="text-[11px] font-bold text-slate-500 uppercase">Nama Ketua</label><input required name="Nama Ketua Group" onChange={(e) => setGrpData({...grpData, [e.target.name]: e.target.value})} className="w-full bg-slate-50 border-2 border-slate-100 rounded-xl px-4 py-3 outline-none focus:border-[#ea580c] text-sm font-bold text-slate-800" /></div>
              <div><label className="text-[11px] font-bold text-slate-500 uppercase">Email</label><input type="email" required name="Email" onChange={(e) => setGrpData({...grpData, [e.target.name]: e.target.value})} className="w-full bg-slate-50 border-2 border-slate-100 rounded-xl px-4 py-3 outline-none focus:border-[#ea580c] text-sm font-bold text-slate-800" /></div>
              <div><label className="text-[11px] font-bold text-slate-500 uppercase">Kategori</label><select name="Kategori Instansi" onChange={(e) => setGrpData({...grpData, [e.target.name]: e.target.value})} className="w-full bg-slate-50 border-2 border-slate-100 rounded-xl px-4 py-3 outline-none focus:border-[#ea580c] text-sm font-bold text-slate-800"><option>Lembaga Pendidikan & Penelitian</option><option>Kementerian & Pemerintah</option><option value="Pemerintah Daerah">Pemerintah Daerah</option><option>Lainnya</option></select></div>
              <div><label className="text-[11px] font-bold text-slate-500 uppercase">Instansi</label><input required name="Nama Instansi" onChange={(e) => setGrpData({...grpData, [e.target.name]: e.target.value})} className="w-full bg-slate-50 border-2 border-slate-100 rounded-xl px-4 py-3 outline-none focus:border-[#ea580c] text-sm font-bold text-slate-800" /></div>
              <div><label className="text-[11px] font-bold text-slate-500 uppercase">Jumlah Orang</label><input type="number" required name="Jumlah Orang" onChange={(e) => setGrpData({...grpData, [e.target.name]: e.target.value})} className="w-full bg-slate-50 border-2 border-slate-100 rounded-xl px-4 py-3 outline-none focus:border-[#ea580c] text-sm font-bold text-slate-800" /></div>
              <div className="flex flex-col gap-3 mt-4">
                <button type="submit" disabled={isLoading} className="w-full bg-[#ea580c] text-white font-extrabold py-3.5 rounded-xl shadow-[0_8px_20px_-6px_rgba(249,115,22,0.6)] flex justify-center uppercase tracking-widest text-sm">{isLoading ? <Loader2 size={20} className="animate-spin" /> : 'KIRIM DATA'}</button>
                <button type="button" onClick={() => setView('pilih')} className="w-full bg-slate-100 text-slate-600 font-bold py-3.5 rounded-xl text-sm tracking-widest uppercase active:scale-95 transition-all">KEMBALI</button>
              </div>
          ...
          {view === 'lainnya' && (
            <form onSubmit={(e) => handleSubmit(e, 'Layanan Lain')} className="grid grid-cols-1 gap-3 pt-2">
              <div><label className="text-[11px] font-bold text-slate-500 uppercase">Nama Lengkap</label><input required name="Nama Lengkap" onChange={(e) => setLainData({...lainData, [e.target.name]: e.target.value})} className="w-full bg-slate-50 border-2 border-slate-100 rounded-xl px-4 py-3 outline-none focus:border-[#ea580c] text-sm font-bold text-slate-800" /></div>
              <div><label className="text-[11px] font-bold text-slate-500 uppercase">Keperluan</label><input required name="Keperluan Khusus" onChange={(e) => setLainData({...lainData, [e.target.name]: e.target.value})} className="w-full bg-slate-50 border-2 border-slate-100 rounded-xl px-4 py-3 outline-none focus:border-[#ea580c] text-sm font-bold text-slate-800" /></div>
              <div className="flex flex-col gap-3 mt-4">
                <button type="submit" disabled={isLoading} className="w-full bg-[#ea580c] text-white font-extrabold py-3.5 rounded-xl shadow-[0_8px_20px_-6px_rgba(249,115,22,0.6)] flex justify-center uppercase tracking-widest text-sm">{isLoading ? <Loader2 size={20} className="animate-spin" /> : 'KIRIM DATA'}</button>
                <button type="button" onClick={() => setView('pilih')} className="w-full bg-slate-100 text-slate-600 font-bold py-3.5 rounded-xl text-sm tracking-widest uppercase active:scale-95 transition-all">KEMBALI</button>
              </div>
